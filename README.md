...
```
User Browser <--WebRTC--> LiveKit Server <--Audio Stream--> Python Agent
                                                                |
                                                                v
...
3. Agent receives audio, sends to Deepgram for transcription
4. Transcribed text passes to Groq LLM for intent classification
5. LLM decides to call a tool (e.g., book_appointment)
6. Agent executes tool, queries/updates Supabase
7. Agent sends TOOL_UPDATE event to frontend via Data Channel
8. Frontend displays visual feedback (card animation)
9. LLM generates natural language response
10. Cartesia converts response to speech
11. Audio streams back to user
```
